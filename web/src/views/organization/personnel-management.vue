<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="所属部门" width="230px" align="center">
        <template slot-scope="{row}">
...
export default {
  ...
  methods: {
    getList() {
      ...
      fetchPersonnelList(this.listQuery).then(response => {
        ...
        for (var i = 0; i < this.personnelList.length; i ++) {  //遍历数组
            if (this.personnelList[i].is_student == 1)  //如果为数字，则返回该元素的值
    ...
    uploadExcel2Import() {
      uploadExcelContent(this.tableData, 0).then(() => {
        this.$notify({
          title: 'Success',
          message: '人员批量创建成功',
          type: 'success',
